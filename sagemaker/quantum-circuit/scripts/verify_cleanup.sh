...
# Verify local analysis files
echo "Checking local analysis files..." | tee -a "$LOG_FILE"
if [ -f pylint_report.txt ] || [ -f codeguru_recommendations.txt ]; then
    echo "Found local files: pylint_report.txt, codeguru_recommendations.txt" | tee -a "$LOG_FILE"
else
    echo "No local analysis files found" | tee -a "$LOG_FILE"
fi
